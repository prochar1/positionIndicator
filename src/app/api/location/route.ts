...
let memoryLocations: { latitude: number; longitude: number; timestamp: number }[] = [];

export async function POST(req: Request) {
  try {
    const data = await req.json();

    if (typeof data.latitude !== "number" || typeof data.longitude !== "number" || typeof data.timestamp !== "number") {
    ...
    if (redis) {
      ...
      await redis.zadd("locations_history", { score: data.timestamp, member: memberData });

      // 2. Remove items older than 24 hours from the sorted set
      await redis.zremrangebyscore("locations_history", "-inf", oldestAllowedTimestamp);
      
      return NextResponse.json({ success: true, storage: "redis" }, { status: 201 });
    } else {
      // In-memory fallback
      memoryLocations.push(data);
      memoryLocations = memoryLocations.filter(loc => loc.timestamp >= oldestAllowedTimestamp);
...
export async function GET() {
  try {
    ...
    if (redis) {
      ...
      const data = await redis.zrange("locations_history", oldestAllowedTimestamp, "+inf", { byScore: true });
      return NextResponse.json(data);
    } else {
      // In-memory fallback
      const validLocations = memoryLocations.filter(loc => loc.timestamp >= oldestAllowedTimestamp);
